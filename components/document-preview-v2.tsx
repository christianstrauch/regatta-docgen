...
import { racingRules, type Fleet, type FleetSpecificProvision } from '@/lib/racing-rules'
...
export function DocumentPreview({
  ...
  const generateNoticeOfRace = () => {
    ...
    if (eventData.dates) markdown += `**Date:** ${eventData.dates}\n\n`
    if (eventData.organizingAuthority) markdown += `**Organizing Authority:** ${eventData.organizingAuthority}\n\n`
    ...
    // 3. CLASSES/DIVISIONS/FLEETS
    if (fleets.length > 0 || eventData.classes) {
      markdown += `## 3. CLASSES AND DIVISIONS\n\n`
      if (fleets.length > 0) {
        markdown += `The following fleets will compete:\n\n`
        fleets.forEach((fleet) => {
          markdown += `- **${fleet.name}**`
          if (fleet.description) markdown += `: ${fleet.description}`
          markdown += `\n`
          
          // Add fleet-specific NOR provisions
          const fleetNorProvisions = fleetProvisions.filter(
            p => p.fleetId === fleet.id && (p.section === 'nor' || p.section === 'both')
          )
          if (fleetNorProvisions.length > 0) {
            fleetNorProvisions.forEach(provision => {
              markdown += `  - ${provision.content}\n`
            })
          }
        })
        markdown += `\n`
      }
      if (eventData.classes) markdown += `${eventData.classes}\n\n`
    }

    // 4. ENTRY AND FEES
    markdown += `## 4. ENTRY AND FEES\n\n`
    if (eventData.registrationLink) markdown += `Entries can be submitted at: ${eventData.registrationLink}\n\n`
    if (eventData.entryFee) markdown += `**Entry Fee:** ${eventData.entryFee}\n\n`

    // 5. SCHEDULE
    markdown += `## 5. SCHEDULE\n\n`
    if (eventData.dates) markdown += `**Race Date:** ${eventData.dates}\n\n`
    ...
    if (eventData.venue || eventData.racingArea) {
      markdown += `## 7. VENUE\n\n`
      if (eventData.venue) markdown += `**Location:** ${eventData.venue}\n\n`
      if (eventData.racingArea) markdown += `**Racing Area:** ${eventData.racingArea}\n\n`
      if (eventData.courseLength) markdown += `**Course Length:** ${eventData.courseLength}\n\n`
    }
    ...
    if (eventData.contactName) markdown += `**Name:** ${eventData.contactName}\n\n`
    if (eventData.contactEmail) markdown += `**Email:** ${eventData.contactEmail}\n\n`
    if (eventData.contactPhone) markdown += `**Phone:** ${eventData.contactPhone}\n\n`

    markdown += `---\n\n*Generated by regatta.club Document Generator*\n`
    return markdown
  }
  ...
  const generateSailingInstructions = () => {
    ...
    if (eventData.organizingAuthority) markdown += ` and ${eventData.organizingAuthority} Sailing Instructions`
    markdown += `.\n\n`
    ...
    // Fleet-specific SI provisions
    if (fleets.length > 0) {
      const fleetSIProvisions = fleetProvisions.filter(
        p => p.section === 'si' || p.section === 'both'
      )
      if (fleetSIProvisions.length > 0) {
        markdown += `### 1.2 FLEET-SPECIFIC PROVISIONS\n\n`
        fleets.forEach((fleet) => {
          const provisions = fleetSIProvisions.filter(p => p.fleetId === fleet.id)
    ...
    if (eventData.racingArea) {
      markdown += `## 6. RACING AREA\n\n${eventData.racingArea}\n\n`
    }

    // 7. THE COURSES
    if (eventData.courseLength) {
      markdown += `## 7. THE COURSES\n\n`
      markdown += `Course length: ${eventData.courseLength}\n\nCourse details will be provided separately.\n\n`
    }

    // 8. MARKS
    markdown += `## 8. MARKS\n\nMarks will be as specified in the course sheet.\n\n`

    // 9. THE START
    markdown += `## 9. THE START\n\nRaces will be started using RRS 26.\n\n`

    // 10. THE FINISH
    markdown += `## 10. THE FINISH\n\nThe finishing line will be between the staff displaying a blue flag on the committee boat and a nearby mark.\n\n`

    // 11. TIME LIMITS
    if (eventData.timeLimit) {
      markdown += `## 11. TIME LIMITS\n\n**Time Limit:** ${eventData.timeLimit}\n\n`
    }

    // 12. PROTESTS AND REQUESTS FOR REDRESS
    markdown += `## 12. PROTESTS AND REQUESTS FOR REDRESS\n\nProtests shall be submitted in accordance with RRS 61.\n\n`

    // 13. SCORING
    if (eventData.scoringSystem) {
      markdown += `## 13. SCORING\n\n${eventData.scoringSystem}\n\n`
    }
    ...
    // AWARDS
    if (eventData.awardsDateTime) {
      markdown += `## AWARDS\n\nAwards will be presented at ${eventData.awardsDateTime}.\n\n`
    }
  ...
  const handlePreviewHTML = (type: 'nor' | 'si') => {
    const markdown = type === 'nor' ? generateNoticeOfRace() : generateSailingInstructions()
...
<head>
  ...
  <title>${eventData.title || 'Document'} - ${type === 'nor' ? 'Notice of Race' : 'Sailing Instructions'}</title>
  <style>
    body {
    ...
    h2 { color: #1e40af; margin-top: 2rem; border-bottom: 1px solid #cbd5e1; padding-bottom: 0.3rem; }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => handlePreviewHTML('nor')}>
                <ExternalLink className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleCopy('nor')}>
                {copiedDoc === 'nor' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              </Button>
        ...
        <CardContent>
          <ScrollArea className="h-[600px] w-full rounded-md border p-4">
            <pre className="text-sm whitespace-pre-wrap font-mono">{norMarkdown}</pre>
          </ScrollArea>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => handlePreviewHTML('si')}>
                <ExternalLink className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleCopy('si')}>
                {copiedDoc === 'si' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              </Button>
        ...
        <CardContent>
          <ScrollArea className="h-[600px] w-full rounded-md border p-4">
            <pre className="text-sm whitespace-pre-wrap font-mono">{siMarkdown}</pre>
          </ScrollArea>
        </CardContent>
      </Card>
    </div>
